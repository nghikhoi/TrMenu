 ...
 * @date 2021/2/4 15:28
 */
class KetherOptEco(val points: ParsedAction<*>) : BaseAction<Boolean>() {
    ...
    override fun process(context: QuestContext.Frame): CompletableFuture<Boolean> {
        return context.newFrame(points).run<Any>().thenApply {
            HookPlugin.getOptEco().hasPoints(context.viewer(), it.toString().toIntOrNull() ?: 0)
        }
    }
    ...
    companion object {
        ...
        fun parser() = ScriptParser.parser {
            KetherOptEco(it.next(ArgTypes.ACTION))
        }
